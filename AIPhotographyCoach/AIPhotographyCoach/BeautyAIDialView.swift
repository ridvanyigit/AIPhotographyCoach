...
// MARK: - Apple Beauty AI Seçim Çarkı (Neon Gül Pembesi Akışı)
struct BeautyAIDialView: View {
    @Binding var selectedMode: BeautyAIMode
    ...
    private let pinkColor = Color(red: 1.0, green: 0.35, blue: 0.75) // Gül Pembesi / Neon Magenta
    
    private var allModes: [BeautyAIMode] { BeautyAIMode.allCases }
    ...
    var body: some View {
        VStack(spacing: 4) {
            ...
            Text(allModes[liveTitleIndex].rawValue)
                ...
                .shadow(color: pinkColor.opacity(0.8), radius: 4)
                    ...
                    ZStack {
                        Circle()
                            ...
                            .overlay(
                                Circle()
                                    .stroke(
                                        isFocused ? pinkColor : Color.white.opacity(0.25),
                                        lineWidth: isFocused ? 2.0 : 1.0
                                    )
                            )
                            .shadow(color: isFocused ? pinkColor.opacity(0.6) : Color.clear, radius: 8)
                        
                        Image(systemName: mode.iconName)
                            .font(.system(size: 18, weight: isFocused ? .bold : .regular))
                            .foregroundColor(isFocused ? pinkColor : .white.opacity(0.75))
                    }
                    .scaleEffect(scale)
                    .opacity(opacity)
